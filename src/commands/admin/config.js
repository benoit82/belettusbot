...
module.exports.run = async (client, message, args) => {
  ...
  const addReact = async role => {
    const msg = await message.channel.send(
      `Configuration pour le rôle ***\`${role}\`***, ajoute les réactions à ce message. L'enregistrement sera fait ${idle /
        1000} secondes sans activité sur ce message.`
    )
    ...
    const msgReactCollector = msg.createReactionCollector(filter, { idle })
    msgReactCollector.on('collect', reaction => {
      const reac = reaction.emoji.id
        ? reaction.emoji.id
    ...
    msgReactCollector.on('end', async () => {
      ...
      msg.delete()
      message.channel.send('réaction `' + role + '` sauvegardé.').then(msg => {
        setTimeout(() => {
          msg.delete()
        }, 3000)
      })
  ...
  if (getSetting === 'react') {
    if (args[1] && args[1] === 'reset') {
      const isRoleInputed = args[2] && client.config.LIST_ROLE.includes(args[2])
      if (isRoleInputed) {
